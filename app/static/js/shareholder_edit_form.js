document.addEventListener("DOMContentLoaded", function () {
    ...
    confirmButton.addEventListener("click", function () {
        ...
        if (errors.length > 0) {
            ...
            tr.innerHTML = `
                <input type="hidden" name="shareholders-${newIndex}-id" value="">
                <input type="hidden" name="shareholders-${newIndex}-is_individual" value="${isIndividual ? "individual" : "legal"}">
                <input type="hidden" name="shareholders-${newIndex}-first_name" value="${firstName}">
                <input type="hidden" name="shareholders-${newIndex}-last_name" value="${lastName}">
                <input type="hidden" name="shareholders-${newIndex}-company_name" value="${companyName}">
                <input type="hidden" name="shareholders-${newIndex}-personal_code" value="${personalCode}">
                <input type="hidden" name="shareholders-${newIndex}-registry_code" value="${legalRegistryCode}">
                <td>${name}</td>
                <td>${isIndividual ? "Füüsiline isik" : "Juriidiline isik"}</td>
                <td>${identifier}</td>
                <td><input type="number" class="form-control" name="shareholders-${newIndex}-share" value="${share}"></td>
                <td><input type="checkbox" name="shareholders-${newIndex}-is_founder"></td>
            `;
